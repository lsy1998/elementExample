  ...
  <div id="postDiv">
    ...
    <template v-else>
      <div v-for="post in posts" 
        @click="showPost($event)" 
        :data-index='post.postId' 
        :key='post.postId'
        ...
        <el-row style="height:100%">
          ...
          <el-col :span="post.rightSpan" style="height:100%;">
            <div style="height:80%;margin:10px 0 20px 10px;background-color:rgb(2, 155, 98);border-radius:5px;color:rgb(2, 155, 98)">
  ...
  methods: {
    ...
    async getAllPost () {
      ...
      try {
        ...
        // 处理帖子数据
        this.posts = response.data.post.map(post => {
          const imgReg = /<img.*?(?:>|\/>)/gi
          const srcReg = /src=[\\'\\"]?([^\\'\\"]*)[\\'\\"]?/i
