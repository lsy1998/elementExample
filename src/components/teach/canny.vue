    ...
    <el-row>
      <el-col :span="8" id='code' style="background-color:white;box-shadow:5px 0 3px -1px rgb(228, 228, 228);">
        ...
        def canny():
            ...
            img = cv2.imread('/root/NodeServer/jiaocheng/bgc.png')
            data = (<input id="num1" ref="num1" v-model="num1" style="width:20px" value="100" type="text" >, <input id="num2" ref="num2" v-model="num2" style="width:20px" value="200" type="text" >)
            if not os.path.exists(f'/root/NodeServer/jiaocheng/{userCount}'):
                os.makedirs(f'/root/NodeServer/jiaocheng/{userCount}')
            cv2.imwrite(f'/root/NodeServer/jiaocheng/{userCount}/Canny.jpg', cv2.Canny(img, *data))
        </code></pre>
        </div>
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      alert(12)
      this.$axios({
        url: 'http://47.115.131.98:888/canny',
        method: 'post',
        data: {
